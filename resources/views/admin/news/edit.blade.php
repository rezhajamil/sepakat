        ...
        <form action="{{ route('admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="flex-auto p-6">
                <div class="flex flex-wrap -mx-3">
                    ...
                    <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                        <div class="mb-4">
                            <label for="headline" class="inline-block mb-2 ml-1 text-xs font-bold text-slate-700 ">Headline
                                <span class="ml-1 text-xs text-gray-500 font-base">(max : 80 karakter)</span></label>
                            <input type="text" name="headline" value="{{ old('headline', $news->title) }}"
                                class="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full appearance-none rounded border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-orange-500 focus:outline-none" />
                            @error('headline')
                                <span class="text-sm italic font-light text-red-600">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="w-full max-w-full px-3 shrink-0 md:flex-0">
                        <div class="mb-4">
                            ...
                            <label for="image"
                                ...
                                <i class="mr-2 fa-solid fa-image"></i><span id="file-name">Ganti Gambar</span>
                            </label>
            ...
            if (input.files) {
                for (var i = 0; i < input.files.length; i++) {
                    ...
                    reader.onload = function(e) {
                        ...
                        // console.log(input.files);
                        preview.prepend(
                            `<img src="${e.target.result}" class="object-cover object-center w-full"/>`
                        );
                    }
